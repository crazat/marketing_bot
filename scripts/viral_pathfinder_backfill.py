"""CLI for backfilling Pathfinder metrics on stored Viral Hunter targets.

Usage:
  python scripts/viral_pathfinder_backfill.py --scan-id 62
  python scripts/viral_pathfinder_backfill.py --scan-id 62 --apply
"""
...
from core_services.viral_pathfinder_backfill import backfill_pathfinder_metrics, default_db_path


def _print_summary(report: dict) -> None:
    mode = "apply" if report.get("apply") else "dry-run"
    print("Pathfinder stored target backfill")
    print(f"  mode: {mode}")
    ...
    print(f"  skipped_no_context: {report.get('skipped_no_context', 0)}")
    print(f"  skipped_no_change: {report.get('skipped_no_change', 0)}")
    print(f"  context_sources: {report.get('context_source_counts', {})}")
    print(f"  axis_tiers: {report.get('axis_tier_counts', {})}")
    print(f"  lens_tiers: {report.get('lens_tier_counts', {})}")
    ...
    if samples:
        ...
        for sample in samples[:10]:
            print(
                ...
                f"lens={sample.get('execution_lens')} "
                f"axis={sample.get('axis_fit_score')}:{sample.get('axis_fit_tier')} "
                f"lens_fit={sample.get('lens_fit_score')}:{sample.get('lens_fit_tier')} "
                f"source={sample.get('context_source')} "
                f"title={sample.get('title')}"
            )


def main() -> int:
    parser = argparse.ArgumentParser(description="Backfill Pathfinder metrics for stored Viral Hunter targets.")
    ...
    parser.add_argument("--include-all", action="store_true", help="Recompute even if fit metrics already exist")
