...
- `needs_ai_retry`는 임시 상태다. 재분석에서 AI 판정 또는 최종 게이트 사유가 생기면 그 결과가
  영속 DB 상태를 반드시 대체해야 하며, 단순 재발견 업데이트만 기존 재시도 상태를 보존한다.
- 교통사고 축은 `교통사고`·`자동차사고`·`후유증`·`자보`·보험·추돌 같은 사고 고유 단서가 있는
  시드만 쓴다. `입원`·`병원`만 있는 모호한 시드는 정신과·요양병원 등 비관련 결과를 끌어오므로
  Pathfinder seed와 Viral 최종 게이트 양쪽에서 차단한다.
