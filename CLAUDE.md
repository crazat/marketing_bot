...
- Handoff audit route bridge는 `cost/consultation/availability/safety` 렌즈가 `community_base`/`*_community`/source-lens 증거를 가진 경우에만 추천·경험·관리방법 같은 concrete route를 인정한다. 단순 `궁금해요/알려주세요` generic 질문을 route-ready로 열면 안 된다. Viral Hunter의 `_ai_target_viral_action_route_matched()`와 audit의 `_viral_action_route_evidence()` 기준을 함께 동기화해야 한다. (2026-07-09 scan 104)
- Viral quality metric backfill은 `score_breakdown`의 `clinic_treatment_fit_score`/`worksite_efficiency_score` 등 quality metric만 채우고 `comment_status`는 절대 바꾸지 않는다. scan 104에서 4,648건 backfill 후 metric coverage 100% 확인. 재실행은 `scripts/viral_quality_metric_backfill.py --scan-id <id> --apply` 사용. (2026-07-09 scan 104)
- Handoff audit `discarded_execution_rescue`는 `current_reject_reason`이 있거나 `filtered_out_ai`인 행을 자동 재큐 후보로 세면 안 된다. `filtered_out*` 중 current reject reason 없는 stale 상태만 auto, `skipped/deleted`는 manual review backlog로만 유지. scan 104에서 auto 44건 AI 재검토 후 51 pending/7 filtered_out_ai, 남은 26건은 manual-only. (2026-07-09 scan 104)
